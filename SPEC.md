...
|---|---|---|---|
| CFTC Socrata API (`publicreporting.cftc.gov`, dataset `6dca-aqww`, Legacy Futures Only) | Weekly net long/short positions by trader category, per market, since inception | None (public; optional free app token removes throttling) | Refresh button shows an inline error banner; dashboard keeps showing last-good data (sample or previously fetched) |
...
Report type: **CFTC Legacy** (not Disaggregated or TFF) — the only report
covering all six target sectors in one consistent schema. Trade-off:
Legacy's "commercial" category blends true hedgers with swap dealers,
which can mute the signal specifically in commodity markets (see §10).
